...
OUTPUT_DIR = ARTIFACTS_DIR / "clarity_deberta_v3_large_256"

# keep 256 for VRAM safety on 6GB; you can try 384 on A100 later
MAX_LENGTH = 256

NUM_EPOCHS = 10
TRAIN_BATCH_SIZE = 4     # large model, keep this small
EVAL_BATCH_SIZE = 16
LEARNING_RATE = 1e-5
...
def main():
    ...
    training_args = TrainingArguments(
        ...
        # We evaluate every epoch but DO NOT save checkpoints
        eval_strategy="epoch",
        save_strategy="no",
        ...
        report_to=[],          # no wandb / TB
        seed=42,
        fp16=True,             # mixed precision
        gradient_accumulation_steps=1,
        # gradient_checkpointing=True,  # you can uncomment if VRAM is tight
    )
    ...
    print("\nFinal eval results (clarity, DeBERTa-v3-Large):", eval_results)

    preds_output = trainer.predict(val_ds)
    preds = preds_output.predictions.argmax(axis=-1)
    y_true = preds_output.label_ids

    print("\nValidation classification report (Clarity, DeBERTa-v3-Large):\n")
    print(
    ...
    print(f"\nSaved DeBERTa-v3-Large clarity model to: {OUTPUT_DIR}")
